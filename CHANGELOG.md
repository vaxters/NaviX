...
- `contracts` module: `Route`, `RouteEntry`, `BackstackSnapshot`, `NavEvent`, `NavLifecycleState`,
  `NavixTelemetry`
- `navix-annotations`: `@RouteDestination`
- `navix-runtime`: `BackstackReducer` (pure reducer), `BackstackStore`, `NavigatorImpl`,
  `NavixHost` (Compose), `NavTransitionSpec`, `NavTransitions`, `rememberNavigator`
- `navix-telemetry`: `NavixTelemetryPipeline`, `NavEventExporter`, `LogcatExporter`, `NoOpExporter`
- `navix-compiler`: KSP processor for route discovery, deep link template parsing,
  `RouteRegistryGenerator`, `DeepLinkHandlerGenerator`
