...
class NavixTelemetryPipelineTest {
    ...
    private fun fakeEvent(type: NavEventType = NavEventType.PUSH) = NavEvent(
        type = type,
        from = null,
        to = null,
        timestampMs = 0L,
    ...
    @Test
    fun onEvent_multipleExporters_allExportersReceiveEveryEvent() = runTest {
        val received1 = mutableListOf<NavEvent>()
        val received2 = mutableListOf<NavEvent>()

        val pipeline = NavixTelemetryPipeline(
            exporters = listOf(
                object : NavEventExporter { override fun export(event: NavEvent) { received1.add(event) } },
                object : NavEventExporter { override fun export(event: NavEvent) { received2.add(event) } },
            ),
    ...
    @Test
    fun onEvent_firstExporterThrows_doesNotBlockSubsequentExporters() = runTest {
        val received = mutableListOf<NavEvent>()

        val pipeline = NavixTelemetryPipeline(
            exporters = listOf(
                object : NavEventExporter { override fun export(event: NavEvent) { throw RuntimeException("oops") } },
                object : NavEventExporter { override fun export(event: NavEvent) { received.add(event) } },
            ),
            dispatcher = StandardTestDispatcher(testScheduler),
        )
    ...
    @Test
    fun export_noOpExporter_doesNotThrow() = runTest {
    ...
    @Test
    fun onEvent_multipleEvents_deliveredInOrder() = runTest {
        val types = mutableListOf<NavEventType>()
        val pipeline = NavixTelemetryPipeline(
            exporters = listOf(
                object : NavEventExporter { override fun export(event: NavEvent) { types.add(event.type) } }
            ),
            dispatcher = StandardTestDispatcher(testScheduler),
        )
    ...
    @Test
    fun replayBuffer_multipleEvents_accumulatesOldestFirst() = runTest {
        val pipeline = NavixTelemetryPipeline(
            exporters = emptyList(),
            dispatcher = StandardTestDispatcher(testScheduler),
        )
    ...
    @Test
    fun replayBuffer_lateSubscriber_receivesAllPreviousEvents() = runTest {
        val pipeline = NavixTelemetryPipeline(
            exporters = emptyList(),
            dispatcher = StandardTestDispatcher(testScheduler),
        )
    ...
    @Test
    fun droppedEventCount_freshPipeline_startsAtZero() = runTest {
        val pipeline = NavixTelemetryPipeline(exporters = emptyList())
        assertEquals(0, pipeline.droppedEventCount.value)
    }

    @Test
    fun onEvent_concurrentCalls_doesNotCorruptReplayBuffer() = runTest {
